...
export const STYLES = {
  // HandDiscコンポーネント
  HAND_DISC: {
    BASE: "w-12 h-12 sm:w-16 sm:h-16 border-4 flex items-center justify-center transition-all",
    CURSOR_DISABLED: "cursor-not-allowed",
    CURSOR_POINTER: "cursor-pointer",
    BORDER_SELECTED: "border-amber-500 bg-amber-100",
    BORDER_DEFAULT: "border-amber-300 bg-yellow-50",
  ...
  // Handコンポーネント
  HAND: {
    CONTAINER_BASE: "p-3 sm:p-4 rounded-lg border-4",
    CONTAINER_INACTIVE: "bg-yellow-50 border-amber-300",
    BG_PULSE: "animate-pulse bg-amber-100",
    TITLE: "mb-2 font-bold text-base sm:text-lg h-6 sm:h-7 text-emerald-900",
    MESSAGE_CONTAINER: "mb-2 h-4 sm:h-5",
    MESSAGE_TEXT: "text-xs sm:text-sm text-amber-700",
    DISCS_CONTAINER: "flex gap-1 sm:gap-2 flex-wrap justify-center",
    DISC_COUNT:
      "text-sm sm:text-base font-semibold mt-2 pt-2 border-t-4 text-emerald-800",
  },
  ...
  // ページレイアウト
  PAGE: {
    MAIN: "min-h-screen flex flex-col items-center justify-center bg-transparent p-4 sm:p-4 md:p-8 overflow-x-hidden",
    TITLE:
      "text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-emerald-900 font-bold mb-2 sm:mb-4 flex items-center justify-center",
    TURN_DISPLAY:
      "mb-2 sm:mb-4 text-base sm:text-lg md:text-xl text-emerald-800 font-semibold",
    GAME_OVER:
      "mb-2 sm:mb-4 text-lg sm:text-xl md:text-2xl text-amber-700 font-bold",
    HANDS_CONTAINER:
      "flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-8 mb-4 sm:mb-8 w-full max-w-[min(100vw-2rem,64rem)] px-2",
    HANDS_ITEM: "flex-1",
    WINNER_MESSAGE:
      "mb-2 sm:mb-4 text-xl sm:text-2xl md:text-3xl font-bold text-emerald-600",
    PLAY_AGAIN_BUTTON:
      "mt-4 sm:mt-6 md:mt-8 px-6 py-3 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-lg shadow-lg transition-colors text-base sm:text-lg md:text-xl",
  },

  // 特殊駒説明セクション
  SPECIAL_DISCS: {
    CONTAINER:
      "mt-4 sm:mt-6 md:mt-8 p-3 sm:p-4 md:p-6 bg-white rounded-lg shadow-lg max-w-4xl w-full border-4 border-amber-300",
    TITLE:
      "text-lg sm:text-xl md:text-2xl font-bold mb-2 sm:mb-3 md:mb-4 text-emerald-900",
    GRID: "grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-3 md:gap-4",
    CARD_BASE: "p-2 sm:p-3 md:p-4 rounded-lg bg-yellow-50",
    CARD_BUTTER: "border-4 border-amber-400",
    CARD_CAT: "border-4 border-emerald-400",
    CARD_BUTTERCAT: "border-4 border-amber-600",
    CARD_HEADER: "flex items-center mb-1 sm:mb-2",
    CARD_EMOJI: "text-xl sm:text-2xl md:text-3xl mr-1 sm:mr-2",
    CARD_TITLE: "text-sm sm:text-base md:text-lg font-bold text-emerald-900",
    CARD_TEXT: "text-xs sm:text-sm text-emerald-800",
  },
